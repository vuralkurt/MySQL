...
select ograd,count(ograd) adet from ogrenci group by ograd;
...
select sinif,cinsiyet,count(cinsiyet) from ogrenci group by cinsiyet,sinif ;

--  Örnek 4: Her türden kaç tane kitap olduğunu listeleyiniz.

 select turno,count(turno) as turKitapSayisi from kitaplar group by turno ;


--  Örnek 5: Her türdeki en fazla sayfa sayısı olan kitapları listeleyiniz.

select turno,(select turadi from islemler where islemler.turno=kitaplar.turno ) as kitabin_turu,max(sayfasayisi) kalin 
from kitaplar group by turno;


--  Örnek 6: Her türdeki en az sayfa sayısı olan kitapları listeleyiniz.
select turno,(select turadi from islemler where islemler.turno=kitaplar.turno ) as kitabin_turu,min(sayfasayisi) kalin 
from kitaplar 
group by turno;


--  Örnek 7***: Her türden kaç tane kitap olduğunu listeleyiniz.(Tür isimleri de olsun)
select turno,(select turadi from islemler where islemler.turno=kitaplar.turno ) as kitabin_turu,count(turno)sayi
 from kitaplar 
 group by turno;